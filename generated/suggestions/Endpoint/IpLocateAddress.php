...
class IpLocateAddress extends BaseEndpoint implements Endpoint
{
    use EndpointTrait;

    public function __construct(IpLocateAddressRequest $requestBody)
    {
    ...
    {
        return '/api/4_1/rs/iplocate/address';
    }

    public function getBody(SerializerInterface $serializer, $streamFactory = null): array
    {
        if ($this->body instanceof IpLocateAddressRequest) {
    ...
    /**
     * @return null|DetectAddressResponse
     *
    ...
    protected function transformResponseBody(string $body, int $status, SerializerInterface $serializer, ?string $contentType = null)
    {
        if (false === is_null($contentType) && (200 === $status && false !== mb_strpos($contentType, 'application/json'))) {
            return $serializer->deserialize($body, 'Glook\Dadata\Generated\Suggestions\Model\DetectAddressResponse', 'json');
        }
